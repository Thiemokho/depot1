...
// 	add_editor_style( array( 'css/editor-style.css', fullhouse_fnc_font_url(), 'genericons/genericons.css' ) );
    ...
    add_image_size('property-large-image', 1110, 650, true);
...
function fullhouse_fnc_get_load_plugins(){
	...
	$plugins[] =(array(
		'name'                     => esc_html__('dsIDXpress IDX','fullhouse'), // The plugin name
	    'slug'                     => 'dsidxpress', // The plugin slug (typically the folder name)
	    'required'                 => true, // If false, the plugin is only 'recommended' instead of required
	));
...
function fullhouse_fnc_registart_widgets_sidebars() {
	...
	register_sidebar( 
	array(
		'name'          => esc_html__( 'Left Topbar' , 'fullhouse'),
		'id'            => 'left-top-bar',
		'description'   => esc_html__( 'Appears on posts and pages in the topbar.', 'fullhouse'),
		'before_widget' => '<aside id="%1$s" class="widget clearfix %2$s">',
		'after_widget'  => '</aside>',
	));
	...
	register_sidebar( 
	array(
		'name'          => esc_html__( 'Header Right sidebar' , 'fullhouse'),
		'id'            => 'header-sidebar-right',
		'description'   => esc_html__( 'Appears on posts and pages in the header.', 'fullhouse'),
		'before_widget' => '<aside id="%1$s" class="widget clearfix %2$s">',
		'after_widget'  => '</aside>',
	));
	...
	register_sidebar( 
	array(
		'name'          => esc_html__( 'Header Left sidebar' , 'fullhouse'),
	...
	register_sidebar( 
	array(
		'name'          => esc_html__( 'IDX Sidebar' , 'fullhouse'),
		'id'            => 'idx-sidebar',
		'description'   => esc_html__( 'Appears on posts and pages in the sidebar', 'fullhouse'),
		'before_widget' => '<aside id="%1$s" class="widget clearfix %2$s">',
		'after_widget'  => '</aside>',
...
function fullhouse_fnc_font_url() {
    ...
    */
    ...
    $poppins = _x( 'on', 'Poppins font: on or off', 'fullhouse' );

    $nunito = _x( 'on', 'Nunito font: on or off', 'fullhouse' );

    $muli = _x( 'on', 'Muli font: on or off', 'fullhouse' );
    ...
    if ( 'off' !== $lora || 'off' !== $lato ) {
        ...
        if ( 'off' !== $muli ) {
            $font_families[] = 'Muli:300,400,600,700,800,900';
        }
...
function fullhouse_fnc_scripts() {
	...
	if( is_rtl() ){
		...
		if( !empty($currentSkin) && $currentSkin != 'default' ){ 
			...
			wp_enqueue_style( 'opalestate-'.$currentSkin.'-style', get_template_directory_uri() . '/css/skins/'.$currentSkin.'/opalestate.css' );
		}else {
			// Load our main stylesheet.
...
if(  in_array( 'js_composer/js_composer.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && class_exists('WPBakeryVisualComposerAbstract') ){
	...
	if ( class_exists('dsIdxGlobals') ) {
		require_once(  get_template_directory() . '/inc/vendors/dsidxpress/function-idx.php' );
	}
...
add_filter( 'upload_mimes', 'fullhouse_enable_extended_upload', 1, 1 );
function fullhouse_enable_extended_upload ( $mime_types = array() ) {
